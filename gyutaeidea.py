...
def perform_analysis_with_stopwords_while(data, tier1_num, tier2_num,
                                          stopwords_list):
  # 초기 root word 빈도와 search volume 계산, 불용어 제거
  root_word_freq = Counter([
      word for sublist in data["kw_split_lemma"] for word in sublist
      if word not in stopwords_list
  ])
  root_word_volumes = {}
  for root_word in root_word_freq.keys():
    total_volume = data[data["kw_split_lemma"].apply(
        lambda x: root_word in x and root_word not in stopwords_list
    )]["search_volume"].sum()
    root_word_volumes[root_word] = total_volume
  sorted_root_word_volumes = sorted(root_word_volumes.items(),
                                    key=lambda x: x[1],
                                    reverse=True)
  ...
  # tier1 반복 (while 구조 사용)
  tier1_idx = 0
  while tier1_idx < tier1_num and sorted_root_word_volumes:
    tier1, tier1_volume = sorted_root_word_volumes.pop(
        0)  # 가장 앞의 원소를 선택하고 목록에서 제거

    # tier1을 포함하는 데이터
    tier1_data = data[data["kw_split_lemma"].apply(
        lambda x: tier1 in x and tier1 not in stopwords_list)]

    # tier2 계산
    tier2_root_word_freq = Counter([
        word for sublist in tier1_data["kw_split_lemma"] for word in sublist
        if word not in stopwords_list
    ])
    tier2_root_word_volumes = {}
    for root_word in tier2_root_word_freq.keys():
      total_volume = tier1_data[tier1_data["kw_split_lemma"].apply(
          lambda x: root_word in x and root_word not in stopwords_list
      )]["search_volume"].sum()
      tier2_root_word_volumes[root_word] = total_volume
    sorted_tier2_root_word_volumes = sorted(tier2_root_word_volumes.items(),
                                            key=lambda x: x[1],
                                            reverse=True)[1:tier2_num + 1]
    ...
    sorted_root_word_volumes = [
        item for item in sorted_root_word_volumes
...
print(
    perform_analysis_with_stopwords_while(
        data, tier1_num, tier2_num,
        stopwords.words("english") + ["para", "for"]))
